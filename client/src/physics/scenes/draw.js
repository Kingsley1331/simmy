...
import { screenWriter, drawLine, drawDot, drawShape, drawArrow } from './display/drawing/drawings';
...
export const draw1 = (canvas) => {
  ...
  forEachShape((i) => {
    ...
    if(Scene.settings.display){ //if(10 < counter && selectedShape === 'play' && vertices[4] && vertices.length === 6) {console.log(vertices[4].isColliding);}
  ...
  screenWriter(bufferCtx, 'x:' + Math.round(mousePos.x) + ',  ' + 'y:' + Math.round(mousePos.y), {x:10, y:20});
